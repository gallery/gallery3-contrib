 ...
 */
class pages_xtra_event_Core {
  static function admin_menu($menu, $theme) {
    // Add a settings link to the admin menu.
    $menu->get("content_menu")
      ->append(Menu::factory("link")
               ->id("pages")
               ->label(t("Pages_xtra Settings"))
               ->url(url::site("admin/pages_xtra")));
  }

  static function site_menu($menu, $theme) {
    $menu_pages = ORM::factory("px_static_page")
    ...
    if (count($menu_pages) > 0) {
      foreach ($menu_pages as $one_page) {
        $menu->add_after("home", Menu::factory("link")
             ...
             ->url(url::site("pages_xtra/show/" . $one_page->name)));
      }
    }
  }
}
