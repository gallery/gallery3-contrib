...
class Item_Model extends ORM_MPTT {
  ...
  public function thumb_path() {
    ...
    if ($this->is_photo() || $this->is_embed()) {
      return $base;
  ...
  public function save() {
    ...
    if ((!empty($this->changed) && $significant_changes) || isset($this->data_file)) {
      ...
      if (!$this->loaded()) {
        ...
        if ($this->is_photo() || $this->is_movie() || $this->is_embed()) {
          if ($this->is_photo() || $this->is_embed()) {
  ...
  public function embed_img($extra_attrs) {
    $v = new View("embedplayer.html");
    $max_size = module::get_var("gallery", "resize_size", 640);
    $width = $this->width;
    $height = $this->height;
    if ($width > $max_size || $height > $max_size) {
  ...
  public function valid_name(Validation $v, $field) {
    ...
    if ($this->is_movie() || $this->is_photo() || $this->is_embed()) {
      if ($this->loaded()) {
  ...
  public function valid_field(Validation $v, $field) {
    ...
    case "type":
      $legal_values = array("album", "photo", "movie", "embed");
      break;
